<template>
    ...
    <Head title="Editar informações de uma filial" />

    <BackButton linkToBack="/branches"/>

    <div class="cnt-add-branch">

        <form class="max-w-lg mx-auto flex flex-col justify-center" @submit.prevent="updateBranch" >
            ...
            <button type="submit" class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Salvar mudanças</button>
        </form>
    </div>
...
<script setup>
    ...
    const props = defineProps({ branch: Object, errors: Object, flash: Object });

    const form = reactive({
        name: props.branch.name,
        address: props.branch.address,
        phone: props.branch.phone,
        lat: props.branch.location.lat,
        long: props.branch.location.long,
    })

    function updateBranch(){

        router.post(`/branches/update/${props.branch.id}`, form);
    }
